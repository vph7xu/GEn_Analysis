...
namespace expconst {
  ...
  // He3 Gas
  static const double he3pressure = 1.013;  //10 atm convert J/cm^3
  static const double he3temp = 305;
  // H2 Gas
  ...
  double ebeam(TString config);     //GeV
  double bbtheta(TString config);   //deg
  double bbdist(TString config);    //m
  double sbstheta(TString config);  //deg
  double sbsdist(TString config);   //m
  double hcaltheta(TString config); //deg
  double hcaldist(TString config);  //m
}
...
class SBSconfig {
 public:
  ...
  TString GetSBSconf()      const { return fSBSconf; }
  ...
  // constructor
  SBSconfig(TString conf, int sbsmag) {
    fSBSconf       = conf;
    fSBSmag        = sbsmag;
  ...
  int Print() {
    std::cout << " -------------------------- "                         << std::endl
	      << " GEN Config: " + GetSBSconf()   << std::endl
 ...
 private:
  TString    fSBSconf;             // SBS configuration number
