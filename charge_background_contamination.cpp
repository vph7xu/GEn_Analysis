...
void charge_background_contamination(const char* filename, const char* printfilename, const char* kin, bool flag_eHCAL_cut){
    ...
    TH1D *h_charge_background = new TH1D("h_charge_background","dx (anti dy cut and ntrack_sbs>0); dx (m)",100,-4,4);
    TH1D *h_neutral_background = new TH1D("h_neutral_background","dx (anti dy cut and ntrack_sbs==0); dx (m)",100,-4,4);
    TH1D *h_all_background = new TH1D("h_all_background","dx (anti dy cut); dx (m)",100,-4,4);
    ...
    for (int i = 0; i<nentries; i++){
        ...
        if(run_num_L<runnum and runnum<run_num_H){
	        if(lookupValue(HelicityCheck,runnum)==1 and lookupValue(MollerQuality,runnum)==1 and ( helicity == -1 or helicity == 1)){
                ...
                bool cut_anti_dy = (dy<dy_ac_L)||(dy_ac_H<dy);
                ...
                bool anti_cut_trP_sbs = P_sbs_L>trP_sbs || trP_sbs>P_sbs_H;
                bool cut_PS = ePS>0.2;
                bool cut_vz = abs(vz)<0.27;
                bool cut_vz_sbs = abs(vz_sbs)<0.27;
                bool cut_corr = /*abs(ptheta - ptheta_cal)<0.02 && abs(pphi - pphi_cal + M_PI)<0.05 && abs(trP_sbs - pN_expect)<0.2 &&*/ abs(vz - vz_sbs)<0.05;
                bool cut_chi2ndf_sbs = chi2ndf_sbs<30; 
                ...
                if(cut_eHCAL and cut_coin and cut_dy){
                    ...
                    if(!cut_sbs_track || !chi2ndf_sbs || !cut_corr){
                        h_w2_no_p_cut_cointime_dy->Fill(W2);
                    }

                    if (cut_sbs_track && cut_corr && cut_chi2ndf_sbs){
                ...
                if(cut_eHCAL and W2<3 and cut_coin){
                    ...
                    if (cut_anti_dy){
                        ...
                        if(ntrack_sbs){
                            ...
                            h_dxdy_charge_background->Fill(dy,dx);
    ...
    if (m_p > 0) {
        ...
        //h_charge_background_copy->Scale(m_ref/m_p);
    }
    ...
    //h_charge_background_copy->SetLineColor(kRed);
    ...
    //h_charge_background_copy->Draw("same hist");
    ...
    c3->cd(4);
    h_dxdy_charge_background->Draw("COLZ");


    c4->cd(1);
    h_all_background->SetLineColor(kAzure+6);
    h_neutral_background->SetLineColor(kBlack);
    h_charge_background->SetLineColor(kViolet+2);
    h_all_background->Draw();
    h_neutral_background->Draw("same");
    h_charge_background->Draw("same");

    c4->cd(2);
    h_all_background->SetLineColor(kAzure+6);
    h_all_background_copy->SetLineColor(kBlack);
    h_charge_background_copy->SetLineColor(kViolet+2);
    h_all_background->Draw();
    ...
    c4->SaveAs(Form("plots/proton_plots_for_%s_eHCAL_cut_%s.pdf)",printfilename,std::to_string(flag_eHCAL_cut).c_str()));
