...
void sbs_tracking_plots(const char* filename, const char* printfilename, const char *kin){
	...
	TH2D *h_ptheta_corr = new TH2D("h_ptheta_corr","h_ptheta_corr",200,0.5,0.7,200,0.5,0.7);
	TH2D *h_pphi_corr = new TH2D("h_pphi_corr","h_pphi_corr",200,2.8,4,200,-0.1,0.4);
	TH2D *h_pmag_corr = new TH2D("h_pmag_corr","h_pmag_corr",200,2,2.7,200,1.8,2.7);
	...
	for (int i = 0; i<nentries; i++){
		...
		//coin_time cut only
		cutcoin_time = abs(coin_time-100)<8;
		if (cutcoin_time) h_W2->Fill(W2);
		
		//coin_time and W2 cuts only
		cutW2_coin_time = abs(W2-0.88)<0.4&&abs(coin_time-100)<8;
		if (cutW2_coin_time) h_dxdy->Fill(dy,dx);

		cutQE = (abs(W2-0.88)<0.4&&abs(coin_time-102)<15&&(sqrt(pow((dx+2.7)/0.3,2)+pow((dy-0.2)/0.2,2)))<1);
		if (cutQE){
	...
	c2->cd(1);
	h_pmag_corr->Draw();
	h_pmag_corr->SetXTitle("pN_expect (rad)");
	h_pmag_corr->SetYTitle("trP_sbs (rad)");

	c3->cd(1);
	h_coin_time->Draw();
	h_coin_time->SetXTitle("coin_time (ns)");
	TLine *line01 = new TLine(98, 0, 98, 275);
    TLine *line02 = new TLine(108, 0, 108, 275);
	...
	c2->cd(2);
	h_delta_pmag->Draw();
	h_delta_pmag->SetXTitle("pN_expect-trP_sbs (rad)");

	c3->cd(2);
	h_W2->Draw();
	h_W2->SetXTitle("W2(GeV2)");
    	TLine *line1 = new TLine(0.88-0.4, 0, 0.88-0.4, 300);
    	TLine *line2 = new TLine(0.88+0.4, 0, 0.88+0.4, 300);
	...
	TEllipse *ellipse = new TEllipse(0.2, -2.7, 0.2, 0.3);
