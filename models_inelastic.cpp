...
TH1D* bkg_model(TH2D *hist2D, double xmin, double xmax, const char* kin, bool flag_eHCAL_cut, double eHCAL_L){
	...
	c->Print(Form("plots/bkg_%s_eHCAL_cut_%s_%f_v1.pdf",kin,std::to_string(flag_eHCAL_cut).c_str(),eHCAL_L));
	c->SaveAs(Form("plots/bkg_%s_eHCAL_cut_%s_%f_v1.png",kin,std::to_string(flag_eHCAL_cut).c_str(),eHCAL_L));
...
std::tuple<TH1D*, TH1D*, TH1D*> sim_bkg_hist(const char* sim_filename, double W2_L_sim, double W2_H_sim, double dy_L_sim, double dy_H_sim, double eHCAL_L_sim){
	TFile* sim_file = TFile::Open(sim_filename);
	...
	return std::make_tuple(h_dx_bkg_n,h_dx_bkg_p,h_dx_bkg);
}


void models_inelastic(const char* filename,const char* sim_filename,const char* sim_bkg_filename,const char* printfilename, const char* kin, bool flag_eHCAL_cut){
	...
	auto [h_bkg_n, h_bkg_p, h_bkg] = sim_bkg_hist(sim_bkg_filename, W2_L, W2_H, dy_L, dy_H, eHCAL_L);
	hist_sim_bkg_p = h_bkg_p;
	hist_sim_bkg_n = h_bkg_n;
	hist_sim_bkg = h_bkg;
	...
	hist_sim_bkg->Scale(1.0/hist_sim_bkg->Integral());
	...
	box1->Draw("same");
	box2->Draw("same");
	c1->SaveAs(Form("plots/%s_dxdy_v1.png",kin));

	TCanvas *ctest = new TCanvas("ctest","ctest",3600,3000);
	ctest->Divide(2,2);
	ctest->cd(1);
	hist_p->Draw();
	ctest->cd(2);
	...
	ctest->cd(3);
	hist_sim_bkg->Draw();
	ctest->SaveAs(Form("plots/%s_sim_scaled_v1.png",kin));


	TF1 *fit_data = new TF1("fit_data", fit_sim_n_bkg_sim, h_dx_W2_cut->GetXaxis()->GetXmin(), h_dx_W2_cut->GetXaxis()->GetXmax(),3);
	...
	hist_sim_bkg->SetFillColorAlpha(7,0.5);
	hist_sim_bkg->SetFillStyle(3003);
	...
	for (int i = 1; i<=nBins; ++i){
		h_dx_sim_n_bkg->SetBinContent(i,(hist_p->GetBinContent(i)+hist_n->GetBinContent(i)+hist_sim_bkg->GetBinContent(i)));	
	}
	...
	inelastic_events = hist_sim_bkg->Integral(hist_sim_bkg->FindBin(dx_L),hist_sim_bkg->FindBin(dx_H)); //integrate background hist to get inelastic events under the neutron peak
	...
	//fit_data->Draw();
    	...
    	outfile.open(Form("txt/%s_inelastic_asymmetry_eHCAL_cut_%s_%f_v1.txt",kin,std::to_string(flag_eHCAL_cut).c_str(),eHCAL_L));
	...
	cfit->Print(Form("plots/models_%s_eHCAL_cut_%s_%f_v1.pdf",kin,std::to_string(flag_eHCAL_cut).c_str(),eHCAL_L));
	cfit->SaveAs(Form("plots/models_%s_eHCAL_cut_%s_%f_v1.png",kin,std::to_string(flag_eHCAL_cut).c_str(),eHCAL_L));
	cfit->SaveAs(Form("plots/models_%s_eHCAL_cut_%s_%f_v1.jpg",kin,std::to_string(flag_eHCAL_cut).c_str(),eHCAL_L));
