...
def smooth_hist(h: np.ndarray, sigma_bins: float = 1.0) -> np.ndarray:
    """
    Gaussian smooth a 1D histogram in *bin units*.
    sigma_bins=0 disables smoothing.
    """
    if sigma_bins <= 0:
        return h
    # build kernel out to ±4 sigma
    r = int(max(1, math.ceil(4.0 * sigma_bins)))
    x = np.arange(-r, r+1, dtype=np.float64)
    k = np.exp(-0.5 * (x / sigma_bins) ** 2)
    k /= k.sum()
    return np.convolve(h, k, mode="same")

def scan_1d_sigma(eval_one, best: CutRanges, args, qa_dir: str):
    ensure_dir(qa_dir)

    params = [
        ("dyL", lambda c: c.dyL, lambda c,v: CutRanges(v, c.dyH, c.W2L, c.W2H, c.eL, c.tL, c.tH), args.dyL_min, args.dyL_max),
        ("dyH", lambda c: c.dyH, lambda c,v: CutRanges(c.dyL, v, c.W2L, c.W2H, c.eL, c.tL, c.tH), args.dyH_min, args.dyH_max),
        ("W2L", lambda c: c.W2L, lambda c,v: CutRanges(c.dyL, c.dyH, v, c.W2H, c.eL, c.tL, c.tH), args.W2L_min, args.W2L_max),
        ("W2H", lambda c: c.W2H, lambda c,v: CutRanges(c.dyL, c.dyH, c.W2L, v, c.eL, c.tL, c.tH), args.W2H_min, args.W2H_max),
        ("eL",  lambda c: c.eL,  lambda c,v: CutRanges(c.dyL, c.dyH, c.W2L, c.W2H, v, c.tL, c.tH),  args.eL_min,  args.eL_max),
        ("tL",  lambda c: c.tL,  lambda c,v: CutRanges(c.dyL, c.dyH, c.W2L, c.W2H, c.eL, v, c.tH),  args.tL_min,  args.tL_max),
        ("tH",  lambda c: c.tH,  lambda c,v: CutRanges(c.dyL, c.dyH, c.W2L, c.W2H, c.eL, c.tL, v),  args.tH_min,  args.tH_max),
    ]

    step = args.cut_step

    for name, getv, setv, vmin, vmax in params:
        xs = grid_values(float(vmin), float(vmax), step)
        ys = []

        for x in xs:
            c = setv(best, float(x))
            if not (c.dyL < c.dyH and c.W2L < c.W2H and c.tL < c.tH):
                ys.append(np.nan); continue
            score, det = eval_one(c)
            if det is None or (not np.isfinite(det.get("sigma_phys", np.nan))):
                ys.append(np.nan)
            else:
                ys.append(float(det["sigma_phys"]))

        ys = np.asarray(ys, dtype=float)

        best_val = float(getv(best))
        nearest_val = float(xs[np.argmin(np.abs(xs - best_val))]) if xs.size else best_val

        plt.figure()
        plt.plot(xs, ys, ".-")
        plt.axvline(best_val, linestyle="--", label="best")
        if abs(nearest_val - best_val) > 1e-9:
            plt.axvline(nearest_val, linestyle=":", label="nearest grid")
        plt.xlabel(name)
...
def compute_fom_and_details(
    ...
    """
    Objective for asymmetry optimization (ignore polarizations):

      1) Fit dx shapes with:
           f(dx) = N [ p(dx) + R n(dx) + Nbg b(dx) ]
         using NNLS on unit-area PDFs p,n,b.

      2) Compute dilution from the fit inside dx window:
           fn = Nn_window / Ntot_window

      3) Compute raw asymmetry counting error from DATA after cuts:
           Araw = (N+ - N-) / (N+ + N-)
           sigma_Araw = 2*sqrt(N+*N-) / (N+ + N-)^(3/2)

      4) Statistical error on physics asymmetry (ignoring polarizations & other factors):
           sigma_phys ~= sigma_Araw / fn

    We MINIMIZE sigma_phys. Since the optimizer is a maximizer, we return:
           score = -sigma_phys
    """
...
def scan_dx_window(eval_one_with_cfg, final_cuts: CutRanges, args, qa_dir: str, base_cfg: ObjectiveConfig):
    ensure_dir(qa_dir)
    step = args.cut_step

    dxLs = grid_values(args.dxL_scan_min, args.dxL_scan_max, step)
    dxHs = grid_values(args.dxH_scan_min, args.dxH_scan_max, step)

    best = None  # (score, dxL, dxH, det)
    rows = []
    ...
    for dxL in dxLs:
        for dxH in dxHs:
            if dxL >= dxH:
                continue
            cfg2 = ObjectiveConfig(
                dx_window=(float(dxL), float(dxH)),
                min_data_events=base_cfg.min_data_events,
                min_template_events=base_cfg.min_template_events,
                dy_min_width=base_cfg.dy_min_width,
                W2_min_width=base_cfg.W2_min_width,
                t_min_width=base_cfg.t_min_width,
                penalty_tight=base_cfg.penalty_tight,
            )
            score, det = eval_one_with_cfg(final_cuts, cfg2)
            if det is None or (not np.isfinite(det.get("sigma_phys", np.nan))):
                continue

            rows.append((dxL, dxH, det["sigma_phys"], det["fn"], det["sigma_Araw"]))
            if best is None or score > best[0]:
                best = (score, dxL, dxH, det)

    # Simple plot: sigma vs dxH for each dxL (can be heavy; start minimal)
    # You can also make a heatmap; keeping it simple here.
    if rows:
        xs = np.array([r[0] for r in rows])
        ys = np.array([r[1] for r in rows])
        zs = np.array([r[2] for r in rows])  # sigma_phys

        plt.figure()
        plt.tricontourf(xs, ys, zs, levels=30)
        plt.xlabel("dxL")
        plt.ylabel("dxH")
        plt.title("sigma_phys over dx window (final non-dx cuts fixed)")
        save_fig(os.path.join(qa_dir, "scan2d_dxwindow_sigma.png"),
                 os.path.join(qa_dir, "scan2d_dxwindow_sigma.pdf"))

    return best


def scan_1d_dx_window(eval_one_with_cfg, final_cuts: CutRanges, args, qa_dir: str, base_cfg: ObjectiveConfig):
    ensure_dir(qa_dir)
    step = args.cut_step  # or use a separate --dx-step

    dxL_vals = grid_values(args.dxL_scan_min, args.dxL_scan_max, step)
    dxH_vals = grid_values(args.dxH_scan_min, args.dxH_scan_max, step)

    # Scan dxL with dxH fixed to current dx_win_high
    ysL = []
    for dxL in dxL_vals:
        if dxL >= base_cfg.dx_window[1]:
            ysL.append(np.nan); continue
        cfg2 = ObjectiveConfig(**{**base_cfg.__dict__, "dx_window": (float(dxL), base_cfg.dx_window[1])})
        score, det = eval_one_with_cfg(final_cuts, cfg2)
        ysL.append(det["sigma_phys"] if det is not None else np.nan)

    plt.figure()
    plt.plot(dxL_vals, ysL, ".-")
    plt.axvline(base_cfg.dx_window[0], linestyle="--", label="current dxL")
    plt.xlabel("dxL")
    plt.ylabel("sigma_phys")
    plt.title("1D scan: dxL (dxH fixed)")
    plt.legend()
    save_fig(os.path.join(qa_dir, "scan1d_dxL.png"),
             os.path.join(qa_dir, "scan1d_dxL.pdf"))

    # Scan dxH with dxL fixed to current dx_win_low
    ysH = []
    for dxH in dxH_vals:
        if base_cfg.dx_window[0] >= dxH:
            ysH.append(np.nan); continue
        cfg2 = ObjectiveConfig(**{**base_cfg.__dict__, "dx_window": (base_cfg.dx_window[0], float(dxH))})
        score, det = eval_one_with_cfg(final_cuts, cfg2)
        ysH.append(det["sigma_phys"] if det is not None else np.nan)

    plt.figure()
    plt.plot(dxH_vals, ysH, ".-")
    plt.axvline(base_cfg.dx_window[1], linestyle="--", label="current dxH")
    plt.xlabel("dxH")
    plt.ylabel("sigma_phys")
    plt.title("1D scan: dxH (dxL fixed)")
    plt.legend()
    save_fig(os.path.join(qa_dir, "scan1d_dxH.png"),
             os.path.join(qa_dir, "scan1d_dxH.pdf"))
...
def parse_args():
    ...
    ap.add_argument("--dx-bins", type=int, default=800)
    ...
    ap.add_argument("--dxH-scan-min", type=float, default=0.2)
    ap.add_argument("--dxH-scan-max", type=float, default=0.6)
...
def main():
    ...
    def eval_one_with_cfg(cuts: CutRanges, cfg_override: ObjectiveConfig):
    ...
    dxD = D[args.br_dx]; dyD = D[args.br_dy]; W2D = D[args.br_W2]; ctD = D[args.br_ct]; eHD = D[args.br_eHCAL]; helD = D[args.br_helicity]
    ...
    if args.method == "random":
        ...
        for i in range(args.n_trials):
            ...
            cuts = snap_cuts(cuts, args.cut_step)

            # snapping can make bounds cross; skip those
            if not (cuts.dyL < cuts.dyH and cuts.W2L < cuts.W2H and cuts.tL < cuts.tH):
                continue
            ...
            if score > best_score:
                ...
                best_details = det
                print(
                    f"\n[trial {i+1}/{args.n_trials}] NEW BEST "
                    f"sigma_phys={det['sigma_phys']:.6g} (score={best_score:.6g}) "
                    f"cuts={best_cuts.as_dict()} fn={det['fn']:.4g} sigma_Araw={det['sigma_Araw']:.4g}"
                )
        ...
        def objective(trial):
            step = args.cut_step
            cuts = CutRanges(
                dyL=trial.suggest_float("dyL", args.dyL_min, args.dyL_max, step=step),
                dyH=trial.suggest_float("dyH", args.dyH_min, args.dyH_max, step=step),
    ...
    if best_cuts is not None and np.isfinite(best_score):
        snapped = snap_cuts(best_cuts, args.cut_step)
        snap_score, snap_det = eval_one(snapped)
        final_cuts, final_score, final_details = snapped, snap_score, snap_det
        ...
        if not args.no_local_refine:
            ...
            if best_grid_cuts is not None and np.isfinite(best_grid_score):
                final_cuts, final_score, final_details = best_grid_cuts, best_grid_score, best_grid_det

    # -----------------------------
    # QA
    # -----------------------------
    if final_cuts is not None and np.isfinite(final_score) and final_details is not None:
    ...
    out = {
        "best_sigma_phys": float(final_details["sigma_phys"]) if (final_details is not None and np.isfinite(final_score)) else float("inf"),
        "best_score": float(final_score) if np.isfinite(final_score) else float("-inf"),
    ...
    if final_cuts is not None and np.isfinite(final_score):
        print("QA plots saved in:", os.path.abspath(args.qa_dir))

    best_dx = scan_dx_window(eval_one_with_cfg, final_cuts, args, args.qa_dir, cfg)
    if best_dx is not None:
        _, dxL_best, dxH_best, det = best_dx
        print(f"\nBest dx window scan: dxL={dxL_best:.3f}, dxH={dxH_best:.3f}, sigma_phys={det['sigma_phys']:.6g}, fn={det['fn']:.4g}, sigma_Araw={det['sigma_Araw']:.4g}")


    # after you compute final_cuts/final_score/final_details
    if final_cuts is not None and np.isfinite(final_score):
        scan_1d_sigma(eval_one, final_cuts, args, args.qa_dir)
        scan_1d_dx_window(eval_one_with_cfg, final_cuts, args, args.qa_dir, cfg)


    dxL, dxH = float(dxL_best), float(dxH_best)

    mD = build_cut_mask(dyD, W2D, eHD, ctD, final_cuts, apply_ct_cut=True)
    mN = build_cut_mask(dyN, W2N, eHN, ctN, final_cuts, apply_ct_cut=False)
    mP = build_cut_mask(dyP, W2P, eHP, ctP, final_cuts, apply_ct_cut=False)

    inwin_D = mD & (dxD >= dxL) & (dxD <= dxH)
    inwin_N = mN & (dxN >= dxL) & (dxN <= dxH)
    inwin_P = mP & (dxP >= dxL) & (dxP <= dxH)

    nData_win = int(np.sum(inwin_D))
    nN_win_sim = int(np.sum(inwin_N))
    nP_win_sim = int(np.sum(inwin_P))

    nB_win = 0
    if bkg_tuple is not None:
        dxB, dyB, W2B, ctB, eHB, wB = bkg_tuple
        mB = build_cut_mask(dyB, W2B, eHB, ctB, final_cuts, apply_ct_cut=False)
        inwin_B = mB & (dxB >= dxL) & (dxB <= dxH)
        nB_win = int(np.sum(inwin_B))

    print("\nCounts in final dx window (after non-dx cuts):")
    print(f"  Data entries:    {nData_win}")
    print(f"  Sim neutron:     {nN_win_sim}")
    print(f"  Sim proton:      {nP_win_sim}")
    print(f"  Bkg entries:     {nB_win}")
    print(f"  Fit Nn_window:   {det['yields']['Nn_window']:.6g}   (from templates+fit)")
    print(f"  Fit Ntot_window: {det['yields']['Ntot_window']:.6g} (from templates+fit)")
