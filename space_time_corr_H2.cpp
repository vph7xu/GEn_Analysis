...
void space_time_corr_H2(const char* filename, const char* printfilename, const char* kin){
	...
	TH2D *h_xHCAL_xSH = new TH2D("h_xHCAL_xSH","xHCAL vs xSH correlation; xSH (m); xHCAL (m)", 100, -1.5, 1.5, 100, -3, 1.5);
	...
	for (int i = 0; i<nentries; i++){
	    ...
	    bool goodChi2 = track_chi2<1000;

	    bool goodSpaceCorr = pow(( (xSH+0.04)*cos(-1.2) + (xHCAL+0.8725)*sin(-1.2) )/2,2) 
	    	+pow((-(xSH+0.04)*sin(-1.2) + (xHCAL+0.8725)*cos(-1.2))/0.15,2) < 1;

	    bool goodBBCal	= abs(eSH+ePS-2.6)<0.5;

	    //if(goodChi2){

		    if (goodTrack && goodVz && goodPS && 
		    	goodEHCAL && goodEoverp && goodCoin && goodDy && goodW2){
		    ...
		    if (goodPS && goodEHCAL && goodCoin && goodSpaceCorr && goodBBCal){

		    	h_xHCAL_xSH->Fill(xSH,xHCAL);
		    ...
		    if (goodTrack &&goodVz && goodPS && 
		    	goodEHCAL && goodEoverp && goodSpot && goodW2 ){
		    ...
		    if (goodTrack && goodPS && 
		    	goodEHCAL && goodEoverp && goodCoin && goodSpot && goodW2){
	...
	auto legW = new TLegend(0.2,0.2,0.88,0.88);
	legW->SetBorderSize(0);
	legW->SetFillStyle(0);
	legW->SetTextSize(0.035);

	auto nAll   = h_W2_all_hit->GetEntries();
	auto n45    = h_W2_4_5hit->GetEntries();
	auto n3     = h_W2_3hit->GetEntries();
	auto n4     = h_W2_4hit->GetEntries();
	auto n5     = h_W2_5hit->GetEntries();

	legW->AddEntry(h_W2_all_hit, Form("All hit (N=%lld)", (Long64_t)nAll), "l");
	legW->AddEntry(h_W2_4_5hit,  Form("4 & 5 hit (N=%lld)", (Long64_t)n45), "l");
	legW->AddEntry(h_W2_3hit,    Form("3 hit (N=%lld)", (Long64_t)n3), "l");
	legW->AddEntry(h_W2_4hit,    Form("4 hit (N=%lld)", (Long64_t)n4), "l");
	legW->AddEntry(h_W2_5hit,    Form("5 hit (N=%lld)", (Long64_t)n5), "l");
	legW->Draw();
