...
void psuedomiss(const char* filename, const char* printfilename, const char *kin){
	...
	TH2D *h_dx_Pperp = new TH2D("h_dx_Pperp","Pperp v dx dist; delta-x (m); pPerp (GeV)",100,-3,4,100,0,1);
	TH2D *h_dx_realPperp = new TH2D("h_dx_realPperp","dx v realPperp dist",100,-3,4,100,0,1);
	TH2D *h_dx_realPpar = new TH2D("h_dx_realPpar","dx v realPpar dist",100,-5,5,250,-3,3);


	TH2D *h_Pperp_W2 = new TH2D("h_Pperp_W2","Pperp v W2 dist",200,0,3,100,0,1);
	TH2D *h_realPperp_W2 = new TH2D("h_realPperp_W2","realPperp v W2 dist",200,0,3,100,0,1);
	TH2D *h_realPpar_W2 = new TH2D("h_realPpar_W2","realPpar v W2 dist",200,0,3,100,-5,5);

	TH1D *h_theta_pq_cut_coin_time_and_dx = new TH1D("h_theta_pq_cut_coin_time_and_dx","theta_pq only with dx and coin cut",100,0,1);
    TH1D *h_Pperp_cut_coin_time_and_dx = new TH1D("h_Pperp_cut_coin_time_and_dx","Pperp only with dx and coin cut",100,0,1);
    TH1D *h_realPperp_cut_coin_time_and_dx = new TH1D("h_realPperp_cut_coin_time_and_dx","realPperp only with dx and coin cut",200,-0.5,2);
    TH1D *h_realPpar_cut_coin_time_and_dx = new TH1D("h_realPpar_cut_coin_time_and_dx","realPpar only with dx and coin cut",200,-5,5);
 
    TH2D *h_dx_Pperp_cut_coin_time_and_dy = new TH2D("h_dx_Pperp_cut_coin_time_and_dy","Pperp v dx dist with coin cut ; delta-x (m) ; pPerp (GeV)",100,-4,3,100,0,1);
    TH2D *h_dx_realPperp_cut_coin_time_and_dx = new TH2D("h_dx_realPperp_cut_coin_time_dy_and_dx","dx v realPperp dist with dx and coin cut",100,0,1,250,-4,3);
    TH2D *h_dx_realPpar_cut_coin_time_and_dx = new TH2D("h_dx_realPpar_cut_coin_time_dy_and_dx","dx v realPpar dist with dx and coin cut",100,0,1,250,-4,3);	
	TH2D *h_Pperp_W2_cut_coin_time_and_dx = new TH2D("h_Pperp_W2_cut_coin_time_dy and_dx","Pperp v W2 dist with dx and coin cut",200,0,3,100,0,1);
	TH2D *h_realPperp_W2_cut_coin_time_and_dx = new TH2D("h_realPperp_W2_cut_coin_time_dy and_dx","realPperp v W2 dist with dx and coin cut",200,-2,4,200,-0.5,2);
	TH2D *h_realPpar_W2_cut_coin_time_and_dx = new TH2D("h_realPpar_W2_cut_coin_time_dy_and_dx","realPpar v W2 dist with dx and coin cut ",200,-2,4,200,-5,5);

	TH1D *h_theta_pq_cut_QE = new TH1D("h_theta_pq_cut_QE","theta_pq with dx , W2 and coin cuts",30,0,0.3);
	TH1D *h_Pperp_cut_QE = new TH1D("h_Pperp_cut_QE","Pperp with dx , W2 and coin cuts",40,0,0.4);
	TH1D *h_realPperp_cut_QE = new TH1D("h_realPperp_cut_QE","realPperp with dx , W2 and coin cuts",40,0,0.4);
	TH1D *h_realPpar_cut_QE = new TH1D("h_realPpar_cut_QE","realPpar with dx , W2 and coin cuts",40,-5,5);
    TH2D *h_dx_Pperp_cut_QE = new TH2D("h_dx_Pperp_cut_QE","Pperp v dx dist with W2, and coin cuts ; delta-x (m) ; pPerp (GeV)",100,-4,3,100,0,1);
    TH2D *h_dx_realPperp_cut_QE = new TH2D("h_dx_realPperp_cut_QE","dx v realPperp dist with dx , W2 and coin cuts",100,0,1,250,-4,3);
    TH2D *h_dx_realPpar_cut_QE = new TH2D("h_dx_realPpar_cut_QE","dx v realPpar dist with dx , W2 and coin cuts",100,0,1,250,-4,3);
    TH2D *h_Pperp_W2_cut_QE = new TH2D("h_Pperp_W2_cut_QE","Pperp v W2 dist with dx , W2 and coin cuts",200,0,W2_H,100,0,0.3);
    TH2D *h_realPperp_W2_cut_QE = new TH2D("h_realPperp_W2_cut_QE","real Pperp v W2 dist with dx , W2 and coin cuts",200,0,W2_H,100,0,0.3);
    TH2D *h_realPpar_W2_cut_QE = new TH2D("h_realPpar_W2_cut_QE","real Ppar v W2 dist with dx , W2 and coin cuts",200,0,W2_H,100,-5,5);
	...
	TH2D *h_dy_Pperp = new TH2D("h_dy_Pperp","Pperp v dy dist ; delta-y (m); pPerp (GeV)",100,-4,3,100,0,1);
	...
	for (int i = 0; i<nentries; i++){	
		...
		bool goodEPS = ePS>0.2;
		bool goodVz = abs(vz)<0.27;
		bool goodEHCAL = eHCAL>eHCAL_L;
		bool goodGlobal = goodEPS && goodVz && goodEHCAL;
		...
		//if (W2_L<W2 and W2<W2_H){
			...
			/*for (int j = 0; j<nblk_HCAL; j++){
				KinE += hcal_clus_e[j]/0.1;//sampling_fractions_each_blk[hcal_clus_id[j]];
		...
		if(coin_time_L<coin_time and coin_time<coin_time_H){ //and dx_L<dx and dx<dx_H and W2_L<W2 and W2<W2_H){
			...
			if (dx_L<dx and dx<dx_H /*and dy_L<dy and dy<dy_H*/){
				...
				if(W2_L<W2 and W2<W2_H /*and dx_L<dx and dx<dx_H and dy<dy_H and dy<dy_L*/){
					...
					//h_dy_Pperp_cut_QE->Fill(dy,Pperp);						
					//h_dx_W2_cut_coin_time->Fill(W2,dx);
				}
			}

			
			h_dx_Pperp_cut_coin_time_and_dy->Fill(dx,Pperp);
			h_dy_Pperp_cut_coin_time_and_dx->Fill(dy,Pperp);

			if(W2_L<W2 and W2<W2_H){
				h_dx_Pperp_cut_QE->Fill(dx,Pperp);
				h_dy_Pperp_cut_QE->Fill(dy,Pperp);
			}
	...
	TCanvas* c10 = new TCanvas("c10","c10",1200,1600);
	TCanvas* c11 = new TCanvas("c11","c11",1200,1600);

    c->Divide(2,2);
	c->cd(1);
	h_theta_pq->Draw("CLOZ");
	c->cd(2);
	h_Pperp->Draw("COLZ");
	c->cd(3);
	...
	//TCutG cutsq1 = CreateSquareCut(-10,dx_L,10,dx_H);
		//Create a box
    TBox* box_dxW2_n1 = new TBox(0,dx_L,3,dx_H);
    box_dxW2_n1->SetFillStyle(0);
    box_dxW2_n1->SetLineColor(kRed);

    c2->Divide(2,2);
    c2->cd(1);
    h_theta_pq_cut_coin_time_and_dx->SetXTitle("theta_pq (rad)");
    h_theta_pq_cut_coin_time_and_dx->Draw("COLZ");
    c2->cd(2);
    h_Pperp_cut_coin_time_and_dx->SetXTitle("Pseudo Pmiss perp (GeV)");
    h_Pperp_cut_coin_time_and_dx->Draw("COLZ");
    c2->cd(3);
    h_dx_W2_cut_coin_time->SetXTitle("W2 (GeV)");
    h_dx_W2_cut_coin_time->SetYTitle("dx (m)");
    h_dx_W2_cut_coin_time->Draw("COLZ");
    //cutsq1->Draw("L");
    box_dxW2_n1->Draw("same");
    c2->cd(4);
    ...
    c10->Divide(2,2);
    c10->cd(1);
    h_dy_Pperp->Draw("COLZ");
    c10->cd(2);
    h_dy_Pperp_cut_QE->Draw("COLZ");
    c10->cd(3);
    h_dy_Pperp_cut_coin_time_and_dx->Draw("COLZ");
