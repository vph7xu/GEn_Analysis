...
// === Ye global-fit (mu_n * GEn / GMn) loaded from CSV ===
// CSV columns: Q2_GeV2, mu_n_GE_over_GM, sigma
void LoadYeRatioCSV(const std::string& csv,
                    std::unique_ptr<TGraph>& gCenter,
                    std::unique_ptr<TGraphAsymmErrors>& gBand)
{
  std::vector<double> x, y, s;
  std::ifstream f(csv);
  if(!f){
    std::cerr << "[Ye] Cannot open CSV: " << csv << std::endl;
  } else {
    std::string line;
    // skip header
    std::getline(f, line);
    while(std::getline(f, line)){
      ...
      s.push_back(std::stod(c));
    }
  }
  const int n = (int)x.size();
  gCenter.reset(new TGraph(n));
  gBand.reset(new TGraphAsymmErrors(n));
  for(int i=0;i<n;i++){
    gCenter->SetPoint(i, x[i], y[i]);
    gBand->SetPoint(i, x[i], y[i]);
    gBand->SetPointError(i, 0.0, 0.0, s[i], s[i]);
  }
  gCenter->SetLineColor(kBlack);
  gCenter->SetLineWidth(3);
  gBand->SetFillColorAlpha(kGray+1, 0.30);
  gBand->SetLineColor(kGray+1);
  gBand->SetLineWidth(0);
}

// Build Ye central curve. If is_ratio=true: mu_n*GE/GM ; else: GE.
TGraph* MakeYeCenter(bool is_ratio) {
  const int nQ2 = 200;
  const double Q2min = 1e-3, Q2max = 12.0;
  const double dQ2 = (Q2max - Q2min) / (nQ2 - 1);

  auto g = new TGraph(nQ2);
  for (int i = 0; i < nQ2; ++i) {
    double Q2 = Q2min + i*dQ2;
    ...
    double y = is_ratio ? ( (GM!=0.0) ? constant::mun*GE/GM : 0.0 ) : GE;
    g->SetPoint(i, Q2, y);
  }
...
void GEN_plotter(){
  ...
  // (Ye) Using CSV-based global fit; no Global_fit() calls here.
  LoadWorldData();
  // --- Load Ye parameterization (mu_n G_E/G_M) from CSV (central + 1σ band) ---
  std::unique_ptr<TGraph>            yeCenter;
  std::unique_ptr<TGraphAsymmErrors> yeBand;
  // Path to CSV generated from Ye's GetFF.py (put it in DB/ or adjust path)
  std::string yeCSV = "DB/ye_global_fit_mu_n_GE_over_GM_corrected.csv";
  LoadYeRatioCSV(yeCSV, yeCenter, yeBand);
  ...
  g_new->Draw("AP");
  if(yeBand) yeBand->Draw("3 same");
  if(yeCenter) yeCenter->Draw("L same");
  //g_Sean->Draw("P same");
  ...
  // (Ye) central curve drawn via yeCenter
  if(yeCenter) yeCenter->Draw("L same");
  ...
  // (Ye) removed: gGEGM_global no longer used
  ...
  legend2->AddEntry(yeCenter.get(),"Global Fit - Ye (central)","l");
  legend2->AddEntry(yeBand.get(),"Global Fit - Ye (1#sigma)","f");
  //legend2->AddEntry(yeBandRatio, "Global Fit - Ye (1#sigma)", "f");
  legend2->SetLineColor(0);
  legend2->Draw("same");

  TLegend *legend4 = new TLegend(0.585,0.734,0.898,0.891);
  // legend4: (no Ye GE curve loaded)
  ...
  yeBandGE = MakeYeBand(false);  // GE band
  TGraph            *yeCenterGE = MakeYeCenter(false);  // GE central


  yeBandGE->Draw("3 same");
  yeCenterGE->Draw("L same");
  ...
  legend4->AddEntry(yeCenterGE, "Global Fit - Ye (central)", "l");
  legend4->AddEntry(yeBandGE ,  "Global Fit - Ye (1#sigma)", "f");

  
  // (Ye) No Global_fit GE curve drawn
  legend3->Draw("same");
  legend4->Draw("same");
  pt->Draw("same");

  // (Ye) removed: gGE_global no longer used
